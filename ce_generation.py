    ...
    if isinstance(ce, OWLObjectIntersectionOf) or isinstance(ce, OWLObjectUnionOf):
        for op in ce.operands():
            if isinstance(op, OWLClass):
                ...
                list_nodetypes = readout_OWLclass(op, list_nodetypes)
...
def generate_cedict_from_ce(ce: OWLClassExpression,  current_id=-1, current_class='', current_mp_id=-1, current_result=list(), dict_class_ids=dict(), random_seed=1):
    ...
    if isinstance(ce, OWLClass):
        ...
        current_result.append(new_mp)
        # iterate over filler
        generate_cedict_from_ce(ce._filler, current_id=current_id, current_class=current_class,
...
def create_graphdict_from_ce(ce: OWLClassExpression, list_of_node_types, list_of_edge_types, metagraph, random_seed=327):
    ...
    '''
    ce_dict = dict()
    ce_dict, dict_class_ids = generate_cedict_from_ce(
        ce, current_result=[], dict_class_ids=dict(), random_seed=random_seed)
    graph_dict = create_graphdict_from_cedict(
        ce_dict, list_of_node_types, list_of_edge_types, metagraph, dict_class_ids, random_seed=random_seed)
    return graph_dict
