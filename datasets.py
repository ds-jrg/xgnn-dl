    ...
    def dict_to_networkx(dict_orig):
        ...
        def remap_indices(new_indices_dict):
            ...
            for node_type, index_dict in new_indices_dict.items():
                ...
                continuous_mapping = {
                    old_index: new_index for new_index, old_index in enumerate(unique_indices)}
        ...
        node_type_to_number = {node_type: i for i,
                               node_type in enumerate(node_types)}
        num_node_types = len(node_types)

        # Step 2: Get max_num_nodes as maximal number of nodes of a type
        max_num_nodes = 0
        for edge_data in graph_dict.values():
            max_num_nodes = max(max_num_nodes, max(
                edge_data[0].max(), edge_data[1].max()).item() + 1)
...
class GraphMotifAugmenter():  # getestet
    ...
    def __init__(self, motif='house', num_motifs=0, orig_graph=None):
        ...
        if orig_graph is not None:
            ...
            num_nodes = 400
            num_edges = 3
            self.orig_graph = GenerateRandomGraph.create_BAGraph_nx(
                num_nodes, num_edges)
        self._graph = copy.deepcopy(self.orig_graph)
        self._list_node_in_motif_or_not = [0]*self.orig_graph.number_of_nodes()
        self._number_nodes_of_orig_graph = self.orig_graph.number_of_nodes()
    ...
    def add_motif(motif, graph):  # getestet, aber geht trotzdem nicht
        ...
        if motif == GraphMotifAugmenter.house_motif:
            labels = [str(node['label'])
                      for _, node in graph.nodes(data=True) if 'label' in node]
            for i in range(1, 4):
                assert str(i) in labels, 'Label ' + str(i) + \
...
class HeteroBAMotifDataset():
    ...
    def _convert_labels_to_node_types(self, change_percent_labels=60):
        ...
        # tests
        assert type_to_classify_str != str(
            self._base_label), (type_to_classify_str, self._base_label, labels)
        node_types = hdata_graph.node_types
        node_types_str = [str(node) for node in node_types]
        assert type_to_classify_str in node_types_str, (
            type_to_classify_str, hdata_graph, self._graph.nodes(data=True))
    ...
    def _add_random_types(self, labels, base_label=None, change_percent=40):
        ...
        for i in range(0, len(labels)):
            assert labels[i] in new_labels, ("The label " + str(
                labels[i]) + " is not in the labels.", new_labels)
            ...
            if node_id in node_ids_with_baselabel:
                if random.random() < change_percent/100:
                    self._graph.add_node(
                        node_id, label=labels[random.randint(0, len(labels)-1)])
                    # data['label'] = labels[random.randint(0, number_labels-1)]
                    changes_nodes += 1
    ...
    def print_statistics_to_dataset(dataset: HeteroData):
        ...
        assert isinstance(
            dataset, HeteroData), "The dataset is not a heterogenous graph."
        ...
        if hasattr(dataset, 'labels'):
            ...
            raise Exception(
                "NotImplemented:The dataset has no labels or node types.")
        for label in labels:
...
def create_hetero_ba_houses(not_house_nodes, houses):
    ...
    listnodetype, label_list = replace_random_zeros_with_one_or_three(
        listnodetype, 0.1)
    ...
    for i in range(4):
        ...
        for ind in range(len(listnodetype)):
            if listnodetype[ind] == i:
                help_list_current_to_new_index.append(
                    [ind, count_ints_until_entry(listnodetype, i, ind)])
        ...
        for type_end_index in range(type_start_index, len(list_different_node_types)):
            ...
            for start_node_index in range(len(listedgeindex[0])):
                ...
                if type_start == type_start_index and type_end == type_end_index:
                    ...
                    new_start_index = new_index(
                        look_up_list_start_node, listedgeindex[0][start_node_index])
                    new_end_index = new_index(
                        look_up_list_end_node, listedgeindex[1][start_node_index])
